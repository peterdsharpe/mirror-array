...
if not ray.is_initialized():
    print("Initializing...")
    ray.init(_temp_dir="/tmp/pdsray", include_dashboard=False)
...
temp_start_rel = 1
temp_end_rel = 1e-8
n_iter = 2e7#1e7#3600 * 76563  # Note: Asymptotic runtime of ~220,000 iterations/second on my machine, 77,000 on SuperCloud.
verbose = False
...
for base_face in base_faces:
    base_face.points *= np.array([tesselation_x_scale, tesselation_y_scale, 1]).reshape((1, -1))
    base_face.rotate_z(90, inplace=True) # Make it wider than tall; might as well reduce distortion for isoceles
...
if use_cached_solution:
    ...
    N_procs = int(ray.available_resources()["CPU"]) * 10
    print(f"Parallelizing on {N_procs} CPUs...")


    # remote_function = ray.remote(functools.partial(optimize_targets, **kwargs))
    remote_function = ray.remote(lambda: optimize_targets(**kwargs))
